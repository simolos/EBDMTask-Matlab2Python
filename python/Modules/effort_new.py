...
def get_ready_phase(i, win, screens, kb, io, expClock, trials,
    ...
    activeKey = ['f'] if flag_MultipleKeyPressed else ['lctrl']
...
def effort_production_phase(keys, i, win, screens, kb, io, expClock,
    ...
    target_effort = float(trials.loc[i, 'effort'])
    ...
    started = EPClock.getTime()                 # to store first onset RT
    clear_events(kb, io)            # drop any buffered events
    tap_key = 'f' if flag_MultipleKeyPressed else 'lctrl'
                ...
                for ev in events:
                    ...
                    if key_name == tap_key:
                        ...
                        if keys == combo | {tap_key} or keys == combo:
                            keypr[f, i] = 1           
                            if started!=None:
                                trials.at[i, ReactionTimeEP] = EPClock.getTime()- started
                                started = None
                        else: 
                            keypr[f, i] = 0
                    else :
...
def feedback_phase(i, win, screens, CURSOR, trials, TaskTimings, expClock, dur, GV=None, Hz=None):
    """
    ...
    if keypr_vec is None or Hz is None or GV is None:
        ...
        mean_onsets = float(np.nanmean(keypr_vec))
        tap_rate_norm = (mean_onsets * float(Hz)) / float(GV)
...
def effort_phase(i, win, screens, kb, io,
    ...
    if flag_MultipleKeyPressed:
        ...
        get_ready_phase(i, win, screens, kb, io, expClock, trials,
                                  flag_MultipleKeyPressed, KEYBOARD_MODE, TaskTimings)
        if trials.loc[i, 'Anticipation_EP'] == 0 :
            effort_production_phase(['f'], i, win, screens, kb, io, expClock,
                                dur, GV, Hz, trials, CURSOR, TaskTimings, keypr, flag_MultipleKeyPressed)
    else:
        get_ready_phase(i, win, screens, kb, io, expClock, trials,
                                  flag_MultipleKeyPressed, KEYBOARD_MODE, TaskTimings)
    ...
    feedback_phase(i, win, screens, CURSOR, trials, TaskTimings, expClock, dur, GV=GV, Hz=Hz)
